...
class TeamRankViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        elegantSlideMenuView.refreshDataBlock = { index in
            print("\(index)")
            if let view_tmp = self.elegantSlideMenuViewArray[index].view as? TeamRankTab {
                if self.elegantSlideMenuViewArray[index].isInitial == 0 {
                    self.elegantSlideMenuViewArray[index].isInitial = 1
                    view_tmp.initial()
                }
            }
        }
